...
Rate_SU=zeros(1,length(TX_sets)); % Will carry the single-user MIMO rate (without interference)
Rate_BS=zeros(1,length(TX_sets));% Will carry the rate with analog-only beamsteering
Rate_BS_BDMA = zeros(1,length(TX_sets));
Rate_HP_cl = zeros(1,length(TX_sets));
Rate_HP_fzf = zeros(1,length(TX_sets));
Rate_HP_schedule = zeros(1,length(TX_sets));
Rate_HP_SLNR = zeros(1,length(TX_sets));
                       ...
                       % [a_TX_schedule,a_RX_schedule, ~,~,H_schedule] = Selectusers(Num_users,Num_users_all,a_TX_all,a_RX_all,Num_paths,H_all);%select Num_users from Num_users_all
                        ...
                        for SNR_dB=SNR_dB_range
                            ...
                            for u=1:1:Num_users
                                ...
                                % Single-user rate
                                Rate_SU(TX_index)=Rate_SU(TX_index)+log2(1+rho*S_channel(1,1)^2)/(ITER);                             
                            end
                            
                            G_BDMA=effective_H(H,Wrf_BDMA,Frf_BDMA);
                            Rate_BS_BDMA(TX_index)=Rate_BS_BDMA(TX_index) + RGH(G_BDMA,eye(size(G_BDMA)),rho)/(ITER);
                            %Rate_HP_fzf(Num_RF_index)=Rate_HP_fzf(Num_RF_index) + RGH(G_fzf,Fbb_fzf,rho)/(ITER);
                            Rate_HP_cl(TX_index)=Rate_HP_cl(TX_index) + RGH(G_cl,Fbb_cl,rho)/(ITER);
                            Rate_HP_fzf(TX_index) = Rate_HP_fzf(TX_index) + RGH(G_fzf,Fbb_fzf,rho)/(ITER);
                            Rate_HP_schedule(TX_index) = Rate_HP_schedule(TX_index) + RGH(G_schedule,Fbb_schedule,rho)/(ITER);
                            Rate_HP_SLNR(TX_index) = Rate_HP_SLNR(TX_index) + RGH(G_cl,Fbb_slnr,rho)/(ITER);
                        end % End of SNR loop
                        %Rate_HP_fzf(Num_RF_index)=Rate_HP_fzf(Num_RF_index) + RGH(G_fzf,Fbb_fzf,rho)/(ITER);
...
%
plot(TX_sets,Rate_SU,'-v','linewidth',1.5);
hold on; plot(TX_sets,Rate_BS_BDMA,'linewidth',1.5);
plot(TX_sets,Rate_HP_fzf,'--o','linewidth',1.5);
plot(TX_sets,Rate_HP_cl,'--','linewidth',1.5);
%plot(TX_sets,Rate_HP_schedule,'linewidth',1.5);
plot(TX_sets,Rate_HP_SLNR,'-','linewidth',1.5);
