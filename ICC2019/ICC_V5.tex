...
\bm{a}(\phi,\theta) =&\frac{1}{\sqrt{N_T}}\left[1,  e^{j\kappa d(\sin\phi \sin\theta +\cos\theta)},  e^{j\kappa d(2\sin\phi \sin\theta +2\cos\theta)}, \right. &&\nonumber\\
...
%In this work, we assume that each scatter only contributes one single propagation path, {\em i.e.} $L_{k,u}=1$, which is a common assumption in the literatures.
