...
for Num_users_index=1:length(Num_user_cluster) % Number of users
    ...
    for Num_RF_index = 1:length(RF_sets)
        ...
        for TX_ant=144  %Number of UPA TX antennas
            ...
            for K = [1]
                m_k = Num_users/K;
                % ----------------------------- Channel Parameters ------------------------
                    Rate_SU=zeros(1,length(paths)); % Will carry the single-user MIMO rate (without interference)
                    Rate_BS=zeros(1,length(paths));% Will carry the rate with analog-only beamsteering
                    Rate_BS_BDMA = zeros(1,length(paths));
                     Rate_HP_cl = zeros(1,length(paths));
                    Rate_HP_fzf = zeros(1,length(paths));
                    Rate_HP_schedule = zeros(1,length(paths));
                    Rate_HP_SLNR = zeros(1,length(paths));
  ...
  %                              Channel_cl(:,:) =H_cl(u,:,:);
                                ...
                                % Single-user rate
                                Rate_SU(Num_paths_index)=Rate_SU(Num_paths_index)+log2(1+rho*S_channel(1,1)^2)/(ITER);
                            ...
                            [~,~,sinr] = RGH(G_BDMA,eye(size(G_BDMA)),rho);
                             sinr_all(:,iter) = sinr ;
                            Rate_BS_BDMA(Num_paths_index)=Rate_BS_BDMA(Num_paths_index) + RGH(G_BDMA,eye(size(G_BDMA)),rho)/(ITER);
                            
                            Rate_HP_fzf(Num_paths_index)=Rate_HP_fzf(Num_paths_index) + RGH(G_fzf,Fbb_fzf,rho)/(ITER);
                            %[~,~,sinr] = RGH(G_fzf,Fbb_fzf,rho);
                           
                            Rate_HP_cl(Num_paths_index)=Rate_HP_cl(Num_paths_index) + RGH(G_cl,Fbb_cl,rho)/(ITER);
                            Rate_HP_schedule(Num_paths_index) = Rate_HP_schedule(Num_paths_index) + RGH(G_cl_new,Fbb_cl_new,rho)/(ITER);
                            Rate_HP_SLNR(Num_paths_index) = Rate_HP_SLNR(Num_paths_index) + RGH(G_cl,Fbb_slnr,rho)/(ITER);
...
% 
plot(paths,Rate_SU,'-v','linewidth',1.5);
hold on; plot(paths,Rate_BS_BDMA,'linewidth',1.5);
plot(paths,Rate_HP_fzf,'--o','linewidth',1.5);
plot(paths,Rate_HP_cl,'--','linewidth',1.5);
plot(paths,Rate_HP_schedule,'linewidth',1.5);
plot(paths,Rate_HP_SLNR,'-','linewidth',1.5);
