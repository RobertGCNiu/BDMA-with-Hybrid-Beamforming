...
for Num_users_index=1:length(Num_user_cluster) % Number of users
    ...
    for Num_RF_index = 1:length(RF_sets)
        ...
        for TX_ant=144  %Number of UPA TX antennas
            ...
            ind_RX_h=repmat([0:1:RX_ant_h-1],1,RX_ant_w);
             cluster_sets = [1,2,4,8,16];
                     Rate_HP_cl = zeros(1,length(cluster_sets));
                    Rate_HP_fzf = zeros(1,length(cluster_sets));
                    Rate_HP_schedule = zeros(1,length(cluster_sets));
                    Rate_HP_SLNR = zeros(1,length(cluster_sets));
            for K_index =1:length(cluster_sets)
                K = cluster_sets(K_index);
                m_k = Num_users/K;
                % ----------------------------- Channel Parameters ------------------------
  ...
  %                              Channel_cl(:,:) =H_cl(u,:,:);
                            ...
                            Rate_HP_cl(K_index)=Rate_HP_cl(K_index) + RGH(G_cl,Fbb_cl,rho)/(ITER);
                            Rate_HP_schedule(K_index) = Rate_HP_schedule(K_index) + RGH(G_schedule,Fbb_schedule,rho)/(ITER);
                            Rate_HP_SLNR(K_index) = Rate_HP_SLNR(K_index) + RGH(G_cl,Fbb_slnr,rho)/(ITER);
...
hold on; plot(cluster_sets,sum(Rate_BS_BDMA)*ones(length(cluster_sets),1),'linewidth',1.5);
plot(cluster_sets,sum(Rate_HP_fzf)/5*ones(length(cluster_sets),1),'--o','linewidth',1.5);
plot(cluster_sets,Rate_HP_cl,'--','linewidth',1.5);
%plot(cluster_sets,Rate_HP_schedule,'-^','linewidth',1.5);
plot(cluster_sets,Rate_HP_SLNR,'-','linewidth',1.5);
% plot(SNR_dB_range,var_fzf_clip,'-','linewidth',1.5);
% plot(SNR_dB_range,var_fzf_bzf,'-','linewidth',1.5);
legend('BDMA', 'ZF (16 RF Chains)', 'BZF', 'BSM')
%legend('signle user','BDMA','full-zf','group','selection', 'SLNR')
xlabel('The number of clusters K')
